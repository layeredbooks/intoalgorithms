...
template <typename Graph, bool verbose = false> class DFSPath {
    Graph g;
    ...
    bool dfs(int v, int v_dest, DFSState &state) {
  ...
  public:
    ...
    const std::vector<int> &get_path() const { return dfs_state.path; }
...
template <typename Graph, bool verbose = false> class DFSAllPaths {
    Graph g;
